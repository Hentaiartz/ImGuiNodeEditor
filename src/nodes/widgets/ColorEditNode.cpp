...
ColorEditNode::ColorEditNode(int id)
    : Node(id, "ColorEdit")
{
...
void ColorEditNode::Draw() {}

void ColorEditNode::SaveExtra(ImGuiTextBuffer& b) const {
    b.appendf("label S \"%s\"\n", m_Label.c_str());
...
void ColorEditNode::LoadExtra(const char* key, const char* line) {
...
void ColorEditNode::DrawProperties() {
    ...
    if (Node::PropSection("Color")) {
        ImGui::ColorEdit4("Color", &m_Color.x, ImGuiColorEditFlags_AlphaBar);
    }
...
void ColorEditNode::RenderPreview(const RenderContext& ctx) {
    ...
    if (m_CustomStyle) {
        ...
        ImGui::ColorEdit4(m_Label.c_str(), &m_Color.x, ImGuiColorEditFlags_AlphaBar);
        ...
        if (wt) {
            ...
            s.var(ImGuiStyleVar_FramePadding, ImVec2(wt->m_Spacing * 0.5f, wt->m_Spacing * 0.3f));
        }
        if (m_Width > 0) ImGui::PushItemWidth(m_Width);
        ImGui::ColorEdit4(m_Label.c_str(), &m_Color.x, ImGuiColorEditFlags_AlphaBar);
        if (m_Width > 0) ImGui::PopItemWidth(); }

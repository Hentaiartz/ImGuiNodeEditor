#include "TabBarNode.h"
#include "Core/Application.h"
#include "Core/PreviewHelpers.h"
#include <imgui.h>
#include <cstdio>

TabBarNode::TabBarNode(int id)
    : ContainerNode(id, "TabBar")
{
}

void TabBarNode::SaveExtra(ImGuiTextBuffer& b) const {
    b.appendf("label S \"%s\"\n", m_Label.c_str());
    b.appendf("align I %d\n", m_Align);
}
void TabBarNode::LoadExtra(const char* key, const char* line) {
    if (strcmp(key, "label") == 0) { char v[256]; if (sscanf(line, " S \"%255[^\"]\"", v) >= 1) m_Label = v; }
    else if (strcmp(key, "align") == 0) { sscanf(line, " I %d", &m_Align); }
}
void TabBarNode::DrawProperties() {
...
void TabBarNode::RenderPreview(const RenderContext& ctx) {

...
InputTextNode::InputTextNode(int id)
    : Node(id, "InputText")
{
...
void InputTextNode::Draw() {}

void InputTextNode::SaveExtra(ImGuiTextBuffer& b) const {
    b.appendf("label S \"%s\"\n", m_Label.c_str());
    b.appendf("text S \"%s\"\n", m_Text.c_str());
    b.appendf("maxLength I %d\n", m_MaxLength);
    b.appendf("password B %d\n", (int)m_Password);
    b.appendf("align I %d\n", m_Align);
    b.appendf("offset V %.1f %.1f\n", m_Offset.x, m_Offset.y);
    b.appendf("width F %.1f\n", m_Width);
    b.appendf("height F %.1f\n", m_Height);
...
void InputTextNode::LoadExtra(const char* key, const char* line) {
    ...
    else if (strcmp(key, "maxLength") == 0) { sscanf(line, " I %d", &m_MaxLength); }
    else if (strcmp(key, "password") == 0) { int v; sscanf(line, " B %d", &v); m_Password = v != 0; }
    ...
    else if (strcmp(key, "height") == 0) { sscanf(line, " F %f", &m_Height); }
...
void InputTextNode::DrawProperties() {
    if (Node::PropSection("Basic")) {
        ...
        if (ImGui::InputText("Label", buf, sizeof(buf)))
            m_Label = buf;
        char txt[1024];
        snprintf(txt, sizeof(txt), "%s", m_Text.c_str());
        if (ImGui::InputText("Text", txt, sizeof(txt)))
            m_Text = txt;
        ImGui::DragFloat("Width (0=auto)", &m_Width, 1.0f, 0.0f, 2000.0f);
        ImGui::DragFloat("Height (0=auto)", &m_Height, 1.0f, 0.0f, 1000.0f);
        ImGui::DragInt("Max length", &m_MaxLength, 1, 1, 4096);
    }
    ...
    if (Node::PropSection("Settings")) {
        ImGui::Checkbox("Password", &m_Password);
    }
    if (Node::PropSection("Style")) {
        ...
        if (m_CustomStyle) {
            ...
            ImGui::DragFloat("Rounding", &m_Rounding, 0.5f, 0.0f, 20.0f);
...
void InputTextNode::RenderPreview(const RenderContext& ctx) {
    RenderHelpers::ApplyWidgetAlign(m_Align, RenderHelpers::GetWidgetWidth(this), m_Offset);
    char buf[1024]; snprintf(buf, sizeof(buf), "%s", m_Text.c_str());
    ImGuiInputTextFlags fl = m_Password ? ImGuiInputTextFlags_Password : ImGuiInputTextFlags_None;
    char lbl[128]; snprintf(lbl, sizeof(lbl), "%s##it%d", m_Label.c_str(), m_Id);
    if (m_CustomStyle) {
        ScopedStyle s;
        ...
        s.var(ImGuiStyleVar_FrameRounding, m_Rounding);
        ...
        if (m_Width > 0 || m_Height > 0) {
            ImVec2 sz(m_Width > 0 ? m_Width : ImGui::GetContentRegionAvail().x, m_Height > 0 ? m_Height : 0);
            if (m_Height > 0) fl |= ImGuiInputTextFlags_Multiline;
            if (ImGui::InputTextMultiline(lbl, buf, sizeof(buf), sz, fl)) m_Text = buf;
        } else { if (ImGui::InputText(lbl, buf, sizeof(buf), fl)) m_Text = buf; }
        ...
        if (wt) s.var(ImGuiStyleVar_FrameRounding, wt->m_Rounding);
        if (m_Width > 0 || m_Height > 0) {
            ImVec2 sz(m_Width > 0 ? m_Width : ImGui::GetContentRegionAvail().x, m_Height > 0 ? m_Height : 0);
            if (m_Height > 0) fl |= ImGuiInputTextFlags_Multiline;
            if (ImGui::InputTextMultiline(lbl, buf, sizeof(buf), sz, fl)) m_Text = buf;
